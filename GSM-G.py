##GSM+graph GNNencoder+multi-label classifier
import os
import time
import math
import random
...
class Multilabel(nn.Module):
    def __init__(self, args,word_vec,params):
        ...
        self.fc1 = nn.Linear(args.num_topic, args.num_labels)
    ...
    def forward(self, batch_data, epoch, **kwargs):
        ...
        _, theta, topic_loss = self.vtm.forward(batch_data, text_bow, y, epoch)  # w： [batch,num_topic]

        logit1 = self.fc1(theta)  # (batch, target_size)
        logit=self.sigmoid(logit1)
...
def get_mlb(fname, vocab, round_num):
    ...
    a = [sorted(set(assess.split(','))) for assess in exp_data[round_data]]
    labels = []
    for i in a:
...
def main(args):
    ...
    for round_num in ['1','2','3']:
        ...
        model = Multilabel(args, word_vec=word_vectors, params=params)
        model.to(device)
